...
import { REPORT_EXPORT_COLUMN_KEYS, REPORT_EXPORT_COLUMNS } from "@shared/exportColumns";
...
export default function Reports() {
  ...
  const [selectedExportColumns, setSelectedExportColumns] = useState<string[]>(() => [...REPORT_EXPORT_COLUMN_KEYS]);
  ...
  async function exportReports(columns = selectedExportColumns, includePersonalData = false) {
    if (columns.length === 0) return toast.error("Seleccione pelo menos uma coluna.");
    const format = pendingExportFormat;
    setExportingFormat(format);
    try {
      const params = new URLSearchParams({ columns: columns.join(","), includePersonalData: includePersonalData ? "true" : "false" });
      await downloadProtectedFile(`/api/reports/export/${format}?${params.toString()}`, `relatorios.${format}`);
  ...
  const reportTerm = reportSearch.trim().toLocaleLowerCase();
  const recentReports = useMemo(() => {
    const sorted = [...reports].sort((first, second) => new Date(second.createdAt).getTime() - new Date(first.createdAt).getTime());
    if (!reportTerm) return sorted.slice(0, 7);
    return sorted.filter((report) => [report.type, report.content ?? "", String(report.id), String(report.activityId)].some((value) => value.toLocaleLowerCase().includes(reportTerm)));
  }, [reports, reportTerm]);
  return <DashboardLayoutCustom>
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">Documentação</p><h1 className="text-3xl font-bold text-slate-900 dark:text-white">Relatórios e atas</h1><p className="mt-1 text-slate-600 dark:text-slate-400">Crie, pré-visualize e descarregue documentos em PDF.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => openExportDialog("pdf")} disabled={exportingFormat !== null}><Download className="mr-2 h-4 w-4" /> PDF</Button><Button variant="outline" onClick={() => openExportDialog("csv")} disabled={exportingFormat !== null}><FileSpreadsheet className="mr-2 h-4 w-4" /> CSV</Button><Button onClick={openGenerate} className="bg-emerald-600 text-white hover:bg-emerald-700"><Plus className="mr-2 h-4 w-4" /> Gerar relatório</Button></div></div>
      <ExportColumnDialog open={exportDialogOpen} onOpenChange={setExportDialogOpen} title={`Exportar relatórios em ${pendingExportFormat.toUpperCase()}`} description="Escolha as colunas que pretende incluir no ficheiro." columns={REPORT_EXPORT_COLUMNS} selected={selectedExportColumns} askPersonalData defaultIncludePersonalData={false} onConfirm={(columns, includePersonalData) => { setSelectedExportColumns(columns); void exportReports(columns, includePersonalData); }} confirmLabel={`Exportar ${pendingExportFormat.toUpperCase()}`} isSubmitting={exportingFormat !== null} />
