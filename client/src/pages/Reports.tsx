...
export default function Reports() {
  ...
  async function exportReports(columns = selectedExportColumns) {
  ...
  return <DashboardLayoutCustom>
    <div className="space-y-6">
      ...
      <ExportColumnDialog open={exportDialogOpen} onOpenChange={setExportDialogOpen} title={`Exportar relatórios em ${pendingExportFormat.toUpperCase()}`} description="Escolha as colunas que pretende incluir no ficheiro." columns={REPORT_EXPORT_COLUMNS} selected={selectedExportColumns} onConfirm={(columns) => { setSelectedExportColumns(columns); void exportReports(columns); }} confirmLabel={`Exportar ${pendingExportFormat.toUpperCase()}`} isSubmitting={exportingFormat !== null} />
