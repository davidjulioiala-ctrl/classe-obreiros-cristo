...
export default function Finances() {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return (expensesQuery.data ?? []).filter((item) => !query || [item.id, item.sequence, item.designation, item.quantity, item.unitPrice, item.totalPrice, dateOnly(item.date), item.recordedBy, item.responsibleName].some((value) => text(value).includes(query)));
