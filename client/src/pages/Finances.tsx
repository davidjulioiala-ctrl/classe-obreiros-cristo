...
export default function Finances() {
  ...
  function startIncomeEdit(item: NonNullable<typeof incomeQuery.data>[number]) { setEditingIncome(item.id); setIncomeForm({ description: item.description, amount: String(item.amount), date: String(item.date).slice(0, 10) }); }
  function startExpenseEdit(item: NonNullable<typeof expensesQuery.data>[number]) { setEditingExpense(item.id); setExpenseForm({ designation: item.designation, quantity: String(item.quantity), unitPrice: String(item.unitPrice), date: String(item.date).slice(0, 10) }); }
  ...
  return <DashboardLayoutCustom><div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">Tesouraria</p><h1 className="text-3xl font-bold text-slate-900 dark:text-white">Finanças</h1><p className="mt-1 text-slate-600 dark:text-slate-400">Registe quotas, receitas e despesas com histórico real.</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><SummaryCard label="Cotas" value={totals.quotaTotal} tone="emerald" /><SummaryCard label="Outras receitas" value={totals.incomeTotal} tone="emerald" /><SummaryCard label="Despesas" value={totals.expenseTotal} tone="red" /><SummaryCard label="Saldo total" value={totals.balance} tone={totals.balance >= 0 ? "emerald" : "red"} /></div></div>
