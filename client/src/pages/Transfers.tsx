...
export default function Transfers() {
  ...
  const [adultProcessName, setAdultProcessName] = useState("");
  ...
  const visibleTransfers = useMemo(() => {
    ...
    const sorted = [...(transfersQuery.data ?? [])].sort((first, second) => new Date(second.createdAt).getTime() - new Date(first.createdAt).getTime());
    if (!term) return sorted.slice(0, 7);
    return sorted.filter((transfer) => [String(transfer.id), transfer.processName ?? "", transfer.toChurch ?? "", transfer.reason ?? "", transfer.status].some((value) => value.toLocaleLowerCase().includes(term)));
  }, [historySearch, transfersQuery.data]);
  ...
  const processAdultTransferBatch = async () => {
    ...
    try {
      ...
      const selectedProcessName = adultProcessName.trim();
      if (!selectedProcessName) { pdfWindow?.close(); return toast.error("Indique um nome para este processo de transferência."); }
      const result = await processAdultTransfers.mutateAsync({ memberIds: selectedForPdf, processName: selectedProcessName, reason: selectedReason, ...(adultToGroupId ? { toGroupId: Number(adultToGroupId) } : {}) });
      await refresh();
  ...
  return <DashboardLayoutCustom><motion.div className="mx-auto w-full max-w-6xl space-y-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <Card className="border-0 shadow-sm dark:bg-slate-800"><CardHeader><CardTitle>Histórico</CardTitle><Input value={historySearch} onChange={(event) => setHistorySearch(event.target.value)} placeholder="Pesquisar por ID, processo, destino, motivo ou estado; a pesquisa mostra todos os resultados" aria-label="Pesquisar histórico de transferências" /></CardHeader><CardContent className="space-y-3">{transfersQuery.isLoading ? <p className="text-sm text-slate-500">A carregar…</p> : visibleTransfers.map((transfer) => { const member = members.find((item) => item.id === transfer.memberId); return <div key={transfer.id} className="flex flex-col gap-3 rounded-lg border border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700"><div><div className="mb-1 flex flex-wrap items-center gap-2"><RecordIdBadge id={transfer.id} /><p className="font-semibold">{member?.name ?? `Membro #${transfer.memberId}`}</p></div><p className="text-sm text-slate-500">{transfer.processName && <><strong>{transfer.processName}</strong> · </>}{transfer.toChurch || "Transferência interna"} · {transfer.reason || "Sem motivo indicado"}</p></div><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{statusLabels[transfer.status] ?? transfer.status}</Badge>{transfer.status === "pendente" && <Button size="sm" variant="outline" onClick={() => void run(() => approveTransfer.mutateAsync({ id: transfer.id }), "Transferência aprovada.")}>Aprovar</Button>}{transfer.status === "aprovada" && <Button size="sm" onClick={() => void run(() => completeTransfer.mutateAsync({ id: transfer.id }), "Transferência concluída.")}>Concluir</Button>}<Button size="icon" variant="outline" className="text-red-600" onClick={() => { if (window.confirm("Eliminar esta transferência?")) void run(() => deleteTransfer.mutateAsync({ id: transfer.id }), "Transferência eliminada."); }} aria-label="Eliminar transferência"><Trash2 className="h-4 w-4" /></Button></div></div>; })}{!transfersQuery.isLoading && !visibleTransfers.length && <p className="rounded-lg bg-slate-50 p-8 text-center text-sm text-slate-500 dark:bg-slate-900">{historySearch.trim() ? "Nenhuma transferência encontrada para a pesquisa." : "Ainda não existem transferências."}</p>}</CardContent></Card>
