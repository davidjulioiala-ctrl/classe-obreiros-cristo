...
import { Calendar, Edit2, Loader2, MapPin, Plus, Trash2, Users, X } from "lucide-react";
...
const blankForm: ActivityForm = { name: "", date: "", startTime: "", endTime: "", location: "", type: "", customType: "", audience: "", theme: "", isReligious: true, hasCommission: false };
...
export default function Activities() {
  ...
  const openEdit = (activity: NonNullable<typeof activitiesQuery.data>[number]) => {
    ...
    setFormData({ name: activity.name, date: format(new Date(activity.date), "yyyy-MM-dd"), startTime: activity.startTime ?? "", endTime: activity.endTime ?? "", location: activity.location ?? "", type: knownTypes.includes(storedType) ? storedType : storedType ? "outros" : "", customType: knownTypes.includes(storedType) ? "" : storedType, audience: activity.audience ?? "", theme: activity.theme ?? "", isReligious: activity.isReligious, hasCommission: activity.hasCommission });
  ...
  const saveActivity = async (event: FormEvent) => {
    ...
    const payload = { name: formData.name.trim(), date: formData.date, startTime: formData.startTime || undefined, endTime: formData.endTime || undefined, location: formData.location || undefined, type: finalType || undefined, audience: formData.audience || undefined, theme: formData.theme || undefined, isReligious: formData.isReligious, hasCommission: formData.hasCommission };
    try {
