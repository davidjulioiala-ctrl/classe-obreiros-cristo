...
export default function AuditAndBackup() {
  ...
  const handleRestore = async (id: number, label: string) => {
    ...
    try {
      ...
      toast.success("Backup restaurado. Os dados visíveis foram actualizados automaticamente.");
    } catch (error) {
  ...
  const selectAllLogs = () => setSelectedLogIds(logs.map((log) => log.id));
  const deselectAllLogs = () => setSelectedLogIds([]);

  const removeManyLogs = async (ids = selectedLogIds, message = "Registos seleccionados eliminados em lote.") => {
    const validIds = ids.filter((id) => logs.some((log) => log.id === id));
    if (!validIds.length || !window.confirm(`Eliminar ${validIds.length} registos de auditoria?`)) return;
    try {
      await deleteManyLogs.mutateAsync({ ids: validIds });
      setSelectedLogIds((current) => current.filter((id) => !validIds.includes(id)));
      await utils.audit.list.invalidate();
      toast.success(message);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Erro ao eliminar registos.");
    }
  };

  const removeAllLogs = async () => {
    if (!logs.length) return;
    await removeManyLogs(logs.map((log) => log.id), "Todos os registos apresentados foram eliminados.");
  };
