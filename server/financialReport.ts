...
export function generateFinancialPdf(payload: FinancialReportPayload) {
  const totals = summarizeFinancialReport(payload);
  const doc = new PDFDocument({ size: "A4", layout: "landscape", margin: 36 });
  const chunks: Buffer[] = [];
  doc.on("data", (chunk: Buffer) => chunks.push(chunk));
  doc.fontSize(18).fillColor("#064e3b").text("Relatório Financeiro", { align: "center" });
  ...
  for (const [label, value] of summary) {
    doc.roundedRect(x, 100, 175, 52, 6).fillAndStroke(label === "Saldo" ? "#dcfce7" : "#f8fafc", "#cbd5e1");
    doc.fillColor("#475569").fontSize(9).text(label, x + 10, 112);
    doc.fillColor(label === "Despesas" ? "#b91c1c" : label === "Saldo" ? "#047857" : "#0f172a").fontSize(13).text(value, x + 10, 128);
    x += 185;
  }
  let y = 185;
