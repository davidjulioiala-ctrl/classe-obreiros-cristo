...
## 2FA obrigatório para todos — correcção crítica
- [x] Reproduzir a falha que impede qualquer utilizador de configurar o 2FA.
- [x] Corrigir a transição entre sessão autenticada, painel de configuração e endpoints 2FA.
- [x] Implementar estado obrigatório de configuração por utilizador, com acesso limitado apenas ao setup e recuperação.
- [x] Garantir que login, códigos de recuperação, cancelamento e reentrada não deixam contas bloqueadas.
- [x] Adicionar regressões para utilizadores comuns, administradores, configuração incompleta e activação concluída.
- [x] Validar testes, typecheck, build, responsividade e publicação.
